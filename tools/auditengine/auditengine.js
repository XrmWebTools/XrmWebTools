...
const XRMWebTools = {
	...
	Sidebar: {
		...
		displayOnlineUsers: (userDetails) => {
			const userListElement = document.getElementById('ID_USERLIST2');
			//
			document.querySelectorAll('#ID_USERLIST2 .list-group-item').forEach(el => el.remove());
			...
			document.getElementById("onlineuserstoday").textContent = `Online users today: ${userDetails.length}`; ;
		},
		...
		createUserListItem: (user) => {
			...
			// Convert first and last login times to percentages of 24 hours
			const firstLoginDate = new Date(`1970-01-01T${user.firstLogin}`);
			const lastLoginDate = new Date(`1970-01-01T${user.lastLogin}`);
			const firstLoginPercent = (firstLoginDate.getHours() * 60 + firstLoginDate.getMinutes()) / 1440 * 100;
			const lastLoginPercent = (lastLoginDate.getHours() * 60 + lastLoginDate.getMinutes()) / 1440 * 100;

			// Create the list item element as a DOM node
			const listItem = document.createElement('div');
			listItem.className = "list-group-item d-flex align-items-center border-0 py-3";
        ...
        <div class="flex-fill ms-4 text-limit">
            <div class="d-flex align-items-center justify-content-between">
                <a href="#" class="d-block text-sm text-heading fw-bold">${user.fullname}</a>
                <span class="text-muted text-xs fw-semibold"></span>
            </div>
          <div class="progress progress-sm my-1 position-relative">
                <div class="progress-bar bg-primary" role="progressbar" style="width:${user.progress}%" aria-valuenow="${user.progress}" aria-valuemin="0" aria-valuemax="100"></div>
                <span class="position-absolute" style="left: ${firstLoginPercent}%; top: 50%; transform: translate(-50%, -50%); width: 8px; height: 8px; background-color: #6e46cc; border-radius: 50%;"></span>
                <span class="position-absolute" style="left: ${lastLoginPercent}%; top: 50%; transform: translate(-50%, -50%); width: 8px; height: 8px; background-color: #6e46cc; border-radius: 50%;"></span>
            </div>
            <div class="d-flex justify-content-between text-xs text-muted text-end mt-1">
                <div><span class="font-weight-bold text-muted">${user.firstLogin}</span></div>
                <div>
                    <p class="card-text text-muted"><time datetime="${user.lastLogin}">${user.lastLogin}</time></p>
                </div>
            </div>
        </div>
	...
	Panel1: {
		...
		ButtonClick_SearchAuditsFromToday: async () => {
			try {
				...
				const audits = await XRMWebTools.WebApi.RetrieveAllAuditsFromTodayWithCustomFilter(0, filter, "250", false);
...
<td class="d-xl-table-cell" ${displayValue !== '-' ? 'style="text-decoration: underline; cursor: pointer;"' : ''}>
	...
	Panel7: {
		loadPluginRegistration: async () => {

			try {
				// Retrieve the list of plugin assemblies, selecting only 'name' and 'pluginassemblyidunique',
				// and order them by name in ascending order
				const assemblies = await XRMWebTools.WebApi.RetrieveWithCustomFilter('pluginassemblies?$select=name,pluginassemblyidunique&$orderby=name asc');
				console.log("assemblies", assemblies);

				// Get the element where the assemblies will be appended
				const ass_table = document.getElementById('ass_table');

				// Loop through each assembly retrieved
				assemblies.value.forEach(assembly => {

					// Create a new div element to represent each plugin assembly
					const ass_testje = document.createElement('div');
					ass_testje.className = "form-item-checkable mt-1"; // Set the class name for styling

					// Set the HTML structure for each assembly item
					ass_testje.innerHTML = `
                <input style="display:none;" class="form-item-check" type="radio" name="filter-version" id="filter-version-1">
                <label class="item w-full" for="filter-version-1" style="cursor: pointer;">
                    <span class="form-item-click d-flex align-items-center border border-primary-hover text-heading p-1 rounded-2">
                        <i class="bi bi-lightning-charge text-lg me-3"></i>
                        <span class="text-body text-sm font-semibold">${assembly.name}</span>
                    </span>
                </label>
            `;

					// Add a click event listener to the label element to alert the assembly name
					ass_testje.addEventListener('click', async () => {

						document.getElementById("id_plugintypes").innerHTML = '';
						//alert(`Assembly Name: ${assembly.name}`);
						//const STEPS = await XRMWebTools.WebApi.RetrieveWithCustomFilter(`sdkmessageprocessingsteps?$filter=contains(name,%27${assembly.name}%27)&$orderby=name%20asc`);
						const pluginTypes = await XRMWebTools.WebApi.RetrieveWithCustomFilter(`plugintypes?$filter=assemblyname eq '${assembly.name}'&$orderby=name asc`);


						console.log(`${assembly.name} pluginTypes`, pluginTypes.value)


						pluginTypes.value.forEach(plugin => {
							const pluginTypeDiv = document.createElement('div');
							pluginTypeDiv.className = "col-lg-4 col-sm-4";

							pluginTypeDiv.innerHTML = `
										<div class="card shadow-4-hover">
											<div class="card-body pb-5">
												<div class="d-flex justify-content-between align-items-center">
													<div><img alt="..." class="w-10 h-10" src="https://clever.webpixels.io/img/social/codepen.svg"></div>
													<div class="text-end">
														<div class="d-flex align-items-center text-success text-sm font-semibold"><i class="bi bi-check-circle-fill me-1"></i> <span>Installed</span></div>
													</div>
												</div>
												<div class="mt-6">
<h6 class="mb-2" style="overflow: hidden;">${plugin.typename}</h6>
													<p class="text-sm text-muted">In certain circumstances and owing to the claims of duty.</p>
												</div>
											</div>
										</div>
						`;
							document.getElementById("id_plugintypes").appendChild(pluginTypeDiv);
						})


					});

					// Append the newly created assembly element to the parent container
					ass_table.appendChild(ass_testje);
	...
	RegisterEvents: async function () {
		...
		//// Sidebar
		document.getElementById('id_pane1_loadonlineusers').addEventListener('click',
		...
		XRMWebTools.toggleOverlay(true, "Retrieving", "STUFF");
		await XRMWebTools.Sidebar.getOnlineUsers();

		await XRMWebTools.Panel1.ButtonClick_SearchAuditsFromToday();

		XRMWebTools.toggleOverlay(false);
		//document.getElementById("id_pane1_loadonlineusers").click();
		...
		document.getElementById("morefiltersclick").addEventListener("click", async function (event) {
			// Prevent the default anchor behavior (e.g., scrolling to top)
			event.preventDefault();
		...
		document.getElementById("pane7_pluginregistration").addEventListener("click", async function (event) {
			// Prevent the default anchor behavior (e.g., scrolling to top)
			event.preventDefault();
			XRMWebTools.toggleOverlay(true, "Retrieving", "Plugin Registraion");
