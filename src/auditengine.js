	...
	var AuditApp = {
		...
		Theme: {
			initializeTheme: function () {
				//alert("initializeTheme")
				let theme = localStorage.getItem("theme");

				const getDefaultTheme = () => theme || (window.matchMedia("(prefers-color-scheme: dark)").matches ? "dark" : "light");
				...
				const setTheme = (newTheme) => {
					...
					document.getElementById('app-extension').setAttribute("data-bs-theme", newTheme);

					//document.getElementById("app_img_logo_darkmode").src = chrome.runtime.getURL("src/img/audit128.png");
					//document.getElementById("app_img_logo_lightmode").src = chrome.runtime.getURL("src/img/audit128.png");
					//document.getElementById("create_logo").src = chrome.runtime.getURL("src/img/create_logo.png");
					//document.getElementById("update_logo").src = chrome.runtime.getURL("src/img/update_logo.png");
					//document.getElementById("delete_logo").src = chrome.runtime.getURL("src/img/delete_logo.png");
					//document.getElementById("access_logo").src = chrome.runtime.getURL("src/img/access_logo.png");
				};

				const updateNavbar = (currentTheme) => {
				...
				const updateSidebar = (currentTheme) => {
				...
				const handleThemeChange = (newTheme) => {
					...
					const label = document.getElementById("toggle_light_dark_label");
				...
				document.getElementById("toggle_light_dark").addEventListener("change", (event) => {
			...
			waitForElement: function (selector, callback) {
				...
				if (element) {
					...
					setTimeout(() => AuditApp.Theme.waitForElement(selector, callback), 100);
				}
			},

			setTheme: function () {
				//AuditApp.Theme.waitForElement('#app-extension', AuditApp.Theme.initializeTheme);
				AuditApp.Theme.initializeTheme();
			}
		},
		CONFIG: {
